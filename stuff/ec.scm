...
; ==========================================================================
; The fundamental comprehension do-of
; ==========================================================================
;
; All eager comprehensions are reduced into do-of and
...
;   The code generation for a :do is delegated to do-of:do.
...
(define-syntax do-of
  (syntax-rules (nested if not and or begin :do let)
    ...
    ; implicit nesting -> fold do-of
    ((do-of q1 q2 etc1 etc ...)
     (do-of q1 (do-of q2 etc1 etc ...)) )

    ; no qualifiers at all -> evaluate cmd once
    ((do-of cmd)
     (begin cmd (if #f #f)) )

; now (do-of q cmd) remains

    ; filter -> make conditional
    ((do-of (if test) cmd)
     (if test (do-of cmd)) )
    ((do-of (not test) cmd)
     (if (not test) (do-of cmd)) )
    ((do-of (and test ...) cmd)
     (if (and test ...) (do-of cmd)) )
    ((do-of (or test ...) cmd)
     (if (or test ...) (do-of cmd)) )

    ; begin -> make a sequence
    ((do-of (begin etc ...) cmd)
     (begin etc ... (do-of cmd)) )

    ; fully decorated :do-generator -> delegate to do-of:do
    ((do-of (:do olet lbs ne1? ilet ne2? lss) cmd)
     (do-of:do cmd (:do olet lbs ne1? ilet ne2? lss)) )

; anything else -> call generator-macro in CPS; reentry at (*)

    ((do-of (g arg1 arg ...) cmd)
     (g (do-of:do cmd) arg1 arg ...) )))
...
; 
;      (do-of (:while (:list x '(1 2)) (= x 1)) (display x)).
;
...
;
;      (do-of (:while (:list x '(1)) #t) (display x))
;
...
; ==========================================================================
; The utility comprehensions fold-of, fold3-of
; ==========================================================================

(define-syntax fold3-of
  (syntax-rules (nested)
    ((fold3-of x0 (nested q1 ...) q etc1 etc2 etc3 etc ...)
     (fold3-of x0 (nested q1 ... q) etc1 etc2 etc3 etc ...) )
    ((fold3-of x0 q1 q2 etc1 etc2 etc3 etc ...)
     (fold3-of x0 (nested q1 q2) etc1 etc2 etc3 etc ...) )
    ((fold3-of x0 expression f1 f2)
     (fold3-of x0 (nested) expression f1 f2) )

    ((fold3-of x0 qualifier expression f1 f2)
...
(define-syntax fold-of
  (syntax-rules (nested)
    ((fold-of x0 (nested q1 ...) q etc1 etc2 etc ...)
     (fold-of x0 (nested q1 ... q) etc1 etc2 etc ...) )
    ((fold-of x0 q1 q2 etc1 etc2 etc ...)
     (fold-of x0 (nested q1 q2) etc1 etc2 etc ...) )
    ((fold-of x0 expression f2)
     (fold-of x0 (nested) expression f2) )

    ((fold-of x0 qualifier expression f2)
     (let ((result x0))
       (do-of qualifier (set! result (f2 expression result)))
       result ))))


; ==========================================================================
; The comprehensions list-of string-ec vector-ec etc.
; ==========================================================================

(define-syntax list-of
  (syntax-rules ()
    ((list-of etc1 etc ...)
     (reverse (fold-of '() etc1 etc ... cons)) )))
...
(define-syntax append-of
  (syntax-rules ()
    ((append-of etc1 etc ...)
     (apply append (list-of etc1 etc ...)) )))

(define-syntax string-of
  (syntax-rules ()
    ((string-of etc1 etc ...)
     (list->string (list-of etc1 etc ...)) )))
...
(define-syntax string-append-of
  (syntax-rules ()
    ((string-append-of etc1 etc ...)
     (apply string-append (list-of etc1 etc ...)) )))

(define-syntax vector-of
  (syntax-rules ()
    ((vector-of etc1 etc ...)
     (list->vector (list-of etc1 etc ...)) )))

; Comment: A similar approach as for string-of can be used for vector-of.
;   However, the space overhead for the intermediate list is much lower
;   than for string-of and as there is no vector-append, the intermediate
;   vectors must be copied explicitly.

(define-syntax vector-of-length-of
  (syntax-rules (nested)
    ((vector-of-length-of k (nested q1 ...) q etc1 etc ...)
     (vector-of-length-of k (nested q1 ... q) etc1 etc ...) )
    ((vector-of-length-of k q1 q2             etc1 etc ...)
     (vector-of-length-of k (nested q1 q2)    etc1 etc ...) )
    ((vector-of-length-of k expression)
     (vector-of-length-of k (nested) expression) )

    ((vector-of-length-of k qualifier expression)
     (let ((len k))
...
(define-syntax min-of
  (syntax-rules ()
    ((min-ec etc1 etc ...)
     (fold3-of (min) etc1 etc ... min min) )))

(define-syntax max-of
  (syntax-rules ()
    ((max-ec etc1 etc ...)
     (fold3-of (max) etc1 etc ... max max) )))

(define-syntax last-of
  (syntax-rules (nested)
    ((last-of default (nested q1 ...) q etc1 etc ...)
     (last-of default (nested q1 ... q) etc1 etc ...) )
    ((last-of default q1 q2             etc1 etc ...)
     (last-of default (nested q1 q2)    etc1 etc ...) )
    ((last-of default expression)
     (last-of default (nested) expression) )

    ((last-of default qualifier expression)
     (let ((result default))
       (do-of qualifier (set! result expression))
       result ))))


; ==========================================================================
; The fundamental early-stopping comprehension first-of
; ==========================================================================

(define-syntax first-of
  (syntax-rules (nested)
    ((first-of default (nested q1 ...) q etc1 etc ...)
     (first-of default (nested q1 ... q) etc1 etc ...) )
    ((first-of default q1 q2             etc1 etc ...)
     (first-of default (nested q1 q2)    etc1 etc ...) )
    ((first-of default expression)
     (first-of default (nested) expression) )

    ((first-of default qualifier expression)
...
; (ec-guarded-do-of stop (nested q ...) cmd)
;   constructs (do-of q ... cmd) where the generators gen in q ... are
;   replaced by (:until gen stop).

(define-syntax ec-guarded-do-of
  (syntax-rules (nested if not and or begin)

    ((ec-guarded-do-of stop (nested (nested q1 ...) q2 ...) cmd)
     (ec-guarded-do-of stop (nested q1 ... q2 ...) cmd) )

    ((ec-guarded-do-of stop (nested (if test) q ...) cmd)
     (if test (ec-guarded-do-of stop (nested q ...) cmd)) )
    ((ec-guarded-do-of stop (nested (not test) q ...) cmd)
     (if (not test) (ec-guarded-do-of stop (nested q ...) cmd)) )
    ((ec-guarded-do-of stop (nested (and test ...) q ...) cmd)
     (if (and test ...) (ec-guarded-do-of stop (nested q ...) cmd)) )
    ((ec-guarded-do-of stop (nested (or test ...) q ...) cmd)
     (if (or test ...) (ec-guarded-do-of stop (nested q ...) cmd)) )

    ((ec-guarded-do-of stop (nested (begin etc ...) q ...) cmd)
     (begin etc ... (ec-guarded-do-of stop (nested q ...) cmd)) )

    ((ec-guarded-do-of stop (nested gen q ...) cmd)
     (do-of 
       (:until gen stop) 
       (ec-guarded-do-of stop (nested q ...) cmd) ))

    ((ec-guarded-do-of stop (nested) cmd)
     (do-of cmd) )))

; Alternative: Instead of modifying the generator with :until, it is
;   possible to use call-with-current-continuation:
;
;   (define-synatx first-of 
;     ...same as above...
;     ((first-of default qualifier expression)
;      (call-with-current-continuation 
;       (lambda (cc)
;        (do-of qualifier (cc expression))
;        default ))) ))
;
...
(define-syntax any-of
  (syntax-rules (nested)
    ((any-of (nested q1 ...) q etc1 etc ...)
     (any-of (nested q1 ... q) etc1 etc ...) )
    ((any-of q1 q2             etc1 etc ...)
     (any-of (nested q1 q2)    etc1 etc ...) )
    ((any-of expression)
     (any-of (nested) expression) )

    ((any-of qualifier expression)
     (first-of #f qualifier (if expression) #t) )))

(define-syntax every-of
  (syntax-rules (nested)
    ((every-of (nested q1 ...) q etc1 etc ...)
     (every-of (nested q1 ... q) etc1 etc ...) )
    ((every-of q1 q2             etc1 etc ...)
     (every-of (nested q1 q2)    etc1 etc ...) )
    ((every-of expression)
     (every-of (nested) expression) )

    ((every-of qualifier expression)
     (first-of #t qualifier (if (not expression)) #f) )))
